server <- function(input, output, session) {
  ...
  observe({
    ...
    varEagle_parameters <-
      case_when(
        input$lovEagle_selectCountry == 'ALL' &
          input$lovEagle_selectSector == 'ALL' ~
    ...
    output$valueBoxes_eagle <- renderUI({
      if (input$radio_realTimeYN == "Yes") {
        ...
        temp_vb <-
          merge(DT_realTime()[eval(parse(text = varEagle_parameters)), .SD[which.max(`Trade Time`)], .(symbol, category)]
                ...
                ,
                all.x = T,
                by = "symbol")[, .(symbol, category, Last, name, growth = (Last / close) -
                                     1)][, head(.SD, NO_OF_VALUE_BOXES)]
        ...
        temp_vb <- DT_hist()[eval(parse(text = varEagle_parameters))
                             , lapply(.SD[order(-date)], function(x)
                               head(x, 2)), symbol][, .(
                                 ...
                                 ,
                                 growth = (close / shift(
                                   close, n = 1L, type = "lead"
                                 )) - 1
                               ), symbol][!is.na(prev_close)][, head(.SD, NO_OF_VALUE_BOXES)]
      ...
      lapply(1:nrow(temp_vb), function(i) {
        icon <-
          if (temp_vb[i]$growth < 0) {
            icon("thumbs-down")
          } else{
            icon("thumbs-up")
          }
        color <- if (temp_vb[i]$growth < 0) {
          "red"
        } else{
          "green"
        }
        valueBox(
          tags$p(format(
            round(temp_vb[i]$value, 0), big.mark = ","
          ), style = "font-size:60%;")
          ,
          tags$p(paste(
            temp_vb[i]$name,
            " ",
            round(100 * temp_vb[i]$growth, 2),
            "%",
            sep = ""
          ), style = "font-size:80%;")
          ,
    ...
    output$box_plotEagle <- renderUI({
      box(
        ...
        width = NULL,
        status = "info",
        title = input$radioEagle_selectCategory
        ,
        renderPlot({
          var_plot()
        }, height = function() {
          var_dynamicHeight() * FACET_ROW_HEIGHT
        })
      )
