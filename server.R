server <- function(input,output,session){
  ...
  observeEvent(input$ab_loadFreshData,{
    ...
    if(input$radio_saveDataYN == "Yes"){
      ...
      saveRDS(cbind(DT_hist_temp,data.frame(timestamp = Sys.time())),file=here::here("100_data_raw-input","DT_hist.Rds"))
    }
  ...
  observeEvent(input$ab_loadSavedData,{
    ...
    confirmSweetAlert(session,inputId='alert_timeStamp',title = "Data Refresh"
                      ,text = paste0("Data was last updated on :"
                                     ,strftime(max(DT_hist_temp$timestamp),format="%d%b%y %H:%M")
                                     ,"\nRefresh it ?")
                      ,type = "question",btn_labels = c("No", "Yes")
                      ,btn_colors = NULL,closeOnClickOutside = FALSE,showCloseButton = FALSE,html = FALSE)
  })
  ...
  observeEvent(input$alert_timeStamp,{
    ...
    if(isTRUE(input$alert_timeStamp)){
      ...
      DT_catchup <- tidyquant::tq_get(unique(DT_hist()$symbol),from =min(DT_hist()[,.(date=max(date)),symbol]$date-2), get = "stock.prices")
      setDT(DT_catchup)
  ...
  observe(
    {
      ...
      output$valueBoxes_myShares <- renderUI({
        ...
        validate(need(DT_myShares()$symbol,"Data is not loaded, please load data using the Data Processing Page"))
